...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  const { market, id } = React.use(params);
  ...
  const defaultContent: ProductMarketContent = {
    title: 'Product Details - Red Project',
  ...
  const marketContent: Record<SupportedMarket, ProductMarketContent> = {
    en: defaultContent,
    ca: {
      ...defaultContent,
      title: 'Product Details - Red Project Canada',
  ...
  // PROJECT B SPECIFIC: Additional validation before adding to cart
  const handleAddToCart = () => {
    if (!product) return;

    // Project B specific: Additional validation before adding to cart
    if (product.stock < 5) {
      alert('Sorry, this product is low in stock and cannot be added to cart.');
      return;
    }
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-1/2">
            ...
            <div className="flex items-center mb-4">
              <span className="text-2xl font-bold text-red-400 mr-4">
                {content.currency}
                {product.price}
              </span>
            ...
            <div className="mb-6">
              ...
              {/* Project B specific - show low stock warning */}
              {product.stock > 0 && product.stock < 5 && (
                <span className="ml-2 text-red-400 text-sm">Low stock!</span>
              )}
            </div>
            ...
            <button
              ...
              className="w-full md:w-auto px-6 py-3 bg-red-600 hover:bg-red-700 rounded text-white transition-colors disabled:opacity-50 mb-6"
            >
              {content.addToCart}
            </button>
              ...
              <div className="border border-red-600 rounded p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p>{market === 'ca' ? 'Montreal' : 'Seattle'}</p>
                  </div>
                </div>
              </div>
            ) : (
